...
public class TrainController {
    ...
    Map<String, String> dummyGet() {
        ...
        long before2018 = LocalDateTime.of(2018, 1, 1, 0, 0)
                .toInstant(ZoneOffset.UTC).toEpochMilli();
        map.put("current-millis", Instant.ofEpochMilli(System.currentTimeMillis())
                .atZone(ZoneId.systemDefault()).toString()
        );
        map.put("id-time-extracted", Instant.ofEpochMilli(before2018 + after2018)
                .atZone(ZoneId.systemDefault()).toString()
        );
        return map;
    }
    ...
    String setSellingTravel(@RequestBody TravelKey request) {
        ...
        try {
            final List<Long> idsCar = Optional.ofNullable(railroadCarTravelMap.get(travelKey))
                ...
                try {
                    ...
                    for (String station : list) {
                        for (String place : railroadCar.getSeats()) {
                            SeatKey seatKey = new SeatKey(travelKey.getRoute(),
                                    travelKey.getStart(),
                                    idCar,
                                    place,
                                    station
                    ...
                    seatsByRailroadCar.set(
                            new RailroadCarTravelKey(request.getRoute(), request.getStart(), idCar),
                            seatPlaces
                    );
            ...
            confMaps.getTravelCDAO()
                    .updateState(travelKey.getStart(), travelKey.getRoute(), TravelState.SELLING);
    ...
    GenericResponse<Long> setupRoute(@RequestBody SetRouteRequest request) {
        final IMap<String, String> stationsMap = confMaps.getStationsMap();
        stationsMap.loadAll(new HashSet<>(request.getStations()), false);

        for (String name : request.getStations()) {
            stationsMap.lock(name);
        }
        try {
            ...
            long id = confMaps.getIdGenRoute().newId();
            confMaps.getRouteStationsMap().set(id, request.getStations());
            confMaps.getRouteDelaysMap().set(id, request.getDelays());
            confMaps.getRouteCDAO().insert(
                    id,
                    request.getStations(),
                    request.getDelays(),
                    request.getPrices()
    ...
    GenericResponse<Long> addRailroadCarTravel(@RequestBody AddRailroadCarTravelRequest railroadCarTravel) {
        final TravelKey travelKey = new TravelKey(
                railroadCarTravel.getRoute(),
                railroadCarTravel.getStart()
        );
        TravelState travelState = Optional.ofNullable(confMaps.getTravelMap().get(travelKey))
                .orElse(TravelState.PREPARING);
        ...
        for (Long railroadCarId : railroadCarTravel.getRailroadCars()) {
            ...
            if (!railroadCarMap.containsKey(railroadCarId)) {
                return new GenericResponse<>(
                        "there is no railroadCar with id:" + railroadCarTravel.getRailroadCars(),
                        null
                );
            }
        ...
        try {
            for (Map.Entry<Long, RailroadCar> entry : mapToApplyTravel.entrySet()) {
                ...
                railroadCarMap.set(
                        ...
                        new RailroadCar(
                                travelKey,
                                railroadCar.getTravelType(),
                                railroadCar.getSeats()
                        )
                );
            }
            confMaps.getRailroadCarCDAO().updateTravels(mapToApplyTravel.keySet(), travelKey);
            confMaps.getRailroadCarTravelMap().set(
                    travelKey,
                    railroadCarTravel.getRailroadCars()
            );
            confMaps.getRailroadCarTravelCDAO().insert(
                    railroadCarTravel.getRoute(),
                    railroadCarTravel.getStart(),
                    railroadCarTravel.getRailroadCars()
            );
    ...
    GenericResponse<List<SeatKey>> findFF(@RequestBody FFacingPlaceRequest request) {
        ...
        for (Long car : confMaps.getRailroadCarTravelMap().get(travelKey)) {
            ...
            Future<List<SeatKey>> result = hz.getExecutorService(TestHZConstants.DEFAULT).submitToKeyOwner(
    ...
    GenericResponse<Integer> ticketRequest(@RequestBody TicketRequest request) {
        ...
        System.out.println("schedule the TicketChecker at:"+System.currentTimeMillis());
        hz.getScheduledExecutorService("default")
                .scheduleOnKeyOwner(new
                                TicketChecker(ticketKey, seatKeys),
                        ticketKey.getPartitionKey(),
                        3,
                        TimeUnit.SECONDS
                );
        return createOk(confMaps.getRidePrice(
                request.getRoute(),
                request.getRailroadCar(),
                idxStart,
                idxEndStation)
    ...
    GenericResponse<String> addTravel(@RequestBody TravelRequest travelRequest) {
        ...
        List<Long> railroadCars = confMaps.getRailroadCarTravelMap()
                .get(travelKey);
        ...
        confMaps.getTravelCDAO().insert(
                travelRequest.getRoute(),
                travelRequest.getStart(),
                travelRequest.getTrainType()
        );
